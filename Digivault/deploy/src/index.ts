...
const hostName = process.env.HOSTN || 'http://localhost:5173/';
...
app.post('/api/documents', authMiddleware, async (c: any) => {
    try {
...
app.delete('/api/documents/:id', authMiddleware, async (c: any) => {
    try {
        const userId = c.user.id;
...
app.get('/api/documents', authMiddleware, (c: any) => {
    const userId = c.user.id;
